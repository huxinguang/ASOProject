...
- (void)willMoveToSuperview:(UIView *)newSuperview
{
    ...
    NSInteger count = _imageArray.count - 1;
    if (count > 0) {
...
- (void)layoutSubviews
{
    ...
    _indexLabel.bounds = CGRectMake(0, 0, 90, 30);
    _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, (IS_X_Series ? 44.f : 0.0f)+40);
    ...
    _deleteButton.frame = CGRectMake(self.bounds.size.width-70, (IS_X_Series ? 44.f : 0.0f)+20, 40, 40);
    _backImageView.frame = CGRectMake(20, (IS_X_Series ? 58.f : 14.0f)+20, 10, 17);
}
...
- (UIImageView *)tempView{
    if (!_tempView) {
        ...
        if (UIDeviceOrientationIsLandscape(orientation)) {//横屏
            ...
            //处理长图,图片太长会导致旋转动画飞掉
            if (tempImageH > kAppScreenHeight) {
...
- (void)setupToolbars
{
    ...
    indexLabel.bounds = CGRectMake(0, 0, 90, 30);
    indexLabel.center = CGPointMake(kAppScreenWidth * 0.5, 30);
    indexLabel.layer.cornerRadius = 15;
    indexLabel.clipsToBounds = YES;
    if (_imageArray.count >= 1) {
        indexLabel.text = [NSString stringWithFormat:@"1/%ld", (long)_imageArray.count];
    ...
    _deleteButton.hidden = !_isNeedDelete;
    ...
    _backImageView.hidden = !_isNeedDelete;
}
...
- (void)deleteImage
{
    if (self.imageArray.count == 1) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(photoBrowser:deleteImageForIndex:)]) {
            [self.delegate photoBrowser:self deleteImageForIndex:self.currentImageIndex];
        }
        return;
    }
    NSMutableArray *arrayM = [NSMutableArray arrayWithArray:self.imageArray];
    [arrayM removeObjectAtIndex:self.currentImageIndex];
...
{
    _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", self.currentImageIndex + 1,self.imageArray.count];
    ...
    if (UIDeviceOrientationIsLandscape(orientation)) {
        ...
        [UIView animateWithDuration:kRotateAnimationDuration delay:0.0f options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            if (IS_X_Series) {
                ...
                self.bounds = CGRectMake(0, 0,  kAppScreenHeight - kAppStatusBarHeight - kAppTabbarSafeBottomMargin, kAppScreenWidth);
            } else {
                self.bounds = CGRectMake(0, 0, kAppScreenHeight, kAppScreenWidth);
            }
            [self setNeedsLayout];
            [self layoutIfNeeded];
        ...
        [UIView animateWithDuration:kRotateAnimationDuration delay:0.0f options:UIViewAnimationOptionBeginFromCurrentState animations:^{
            ...
            if (IS_X_Series) {
                self.bounds = CGRectMake(0, 0, kAppScreenWidth, kAppScreenHeight - kAppStatusBarHeight - kAppTabbarSafeBottomMargin);
            } else {
                self.bounds = CGRectMake(0, 0, kAppScreenWidth, kAppScreenHeight);
            }
...
- (id)highQualityImageURLForIndex:(NSInteger)index
{
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    _indexLabel.text = [NSString stringWithFormat:@"%d/%ld", index + 1, (long)_imageArray.count];
    long left = index - 1;
    long right = index + 1;
    left = left>0?left : 0;
    right = right>_imageArray.count?_imageArray.count:right;
...
- (void)show
{
    ...
    if (IS_X_Series) {
        self.frame = CGRectMake(0, kAppStatusBarHeight,kAppScreenWidth,kAppScreenHeight - kAppStatusBarHeight - kAppTabbarSafeBottomMargin);
    } else {
