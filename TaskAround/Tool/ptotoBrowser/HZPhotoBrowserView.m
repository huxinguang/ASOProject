...
- (void)setImageWithURL:(id)url placeholderImage:(UIImage *)placeholder
{
    if (_reloadButton) {
...
//    UIImage *image = [UIImage imageWithContentsOfFile:url];
    //HZWebImage加载图片
    if ([url isMemberOfClass:UIImage.class]) {
        _imageview.image = url;
        [self setNeedsLayout];
    }else if ([url isKindOfClass:NSString.class]) {
        if ([(NSString *)url containsString:@"http"]){
            //添加进度指示器
            ...
            _imageview.image = [UIImage imageWithContentsOfFile:url];
            [self setNeedsLayout];
        }
    }
}
