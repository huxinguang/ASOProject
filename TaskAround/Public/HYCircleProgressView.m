...
- (void)setProgressCircleLine
{
    ...
    self.path  = [UIBezierPath bezierPathWithArcCenter:CGPointMake(self.center.x - self.frame.origin.x, self.center.y - self.frame.origin.y)
...
- (void)setProgress:(CGFloat)percentage animated:(BOOL)animated
{
    ...
    float kDuration=[self.longTime floatValue];
...
- (void)setLongTime:(NSString *)longTime{
    _longTime=longTime;
}
- (void)stopProgress {
