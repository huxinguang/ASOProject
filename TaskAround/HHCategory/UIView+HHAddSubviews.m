...
{
    objc_setAssociatedObject(self, textFieldMaxCharKey, @(maxCharacters), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
- (void)addBottomLine:(UIColor *)lineColor
{
    ...
    layer.backgroundColor = lineColor ? lineColor.CGColor : [UIColor lightGrayColor].CGColor;
...
- (void)hh_addGestureType:(HHGestureType)type gesture:(void (^)(UIGestureRecognizer *))gesture action:(void (^)(UIGestureRecognizer *gesture))block
{
    ...
    [self.actionDict setObject:block forKey:KViewAddress(gestureRec)];
}
